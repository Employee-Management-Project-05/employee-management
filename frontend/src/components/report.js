...
export default class Report extends Component {
  ...
  printDocument() {
    ...
    html2canvas(input).then((canvas) => {
      ...
      doc.setFontSize(28);
      doc.text(85, 10, "Employee");
      doc.setTextColor(0, 0, 255);
      doc.setFontSize(16);
      doc.text(10, 70, "Employee Management");
      ...
      const dateStr =
        "EmployeeManagement_" + date[0] + date[1] + date[2] + date[3] + date[4];
      doc.save(`report_${dateStr}.pdf`);
    });
  }

  componentDidMount() {
    axios.get("http://localhost:8070/employee").then((response) => {
          ...
          <Table stickyHeader aria-label="sticky table">
            ...
            <TableBody>
              {this.state?.ReportData?.map((p, index) => {
                return (
                  <TableRow key={index}>
                    <TableCell align="right">{p.employeeID}</TableCell>
                    <TableCell align="right">{p.name}</TableCell>
                    <TableCell align="right">{p.nic}</TableCell>
                    <TableCell align="right">{p.address}</TableCell>
                    <TableCell align="right">+94 {p.phoneNO}</TableCell>
                  </TableRow>
                );
              })}
            </TableBody>
